...
type G3NRenderer struct {
	...
	bodyNodes map[body.ID]*core.Node
...
func NewG3NRenderer() *G3NRenderer {
	return &G3NRenderer{
		bodyNodes: make(map[body.ID]*core.Node),
		width:     800,
		height:    600,
		running:   false,
...
func (r *G3NRenderer) Initialize() error {
	...
	r.app.Gls().ClearColor(0.1, 0.1, 0.1, 1.0)
	...
	r.camera.SetPosition(0, 5, 15) // Posiziona la camera più in alto e più lontana
	...
	ambientLight := light.NewAmbient(&math32.Color{0.7, 0.7, 0.7}, 1.0) // Aumenta l'intensità della luce ambientale
	r.scene.Add(ambientLight)

	// Aggiungi più luci puntuali per illuminare la scena da diverse angolazioni
	pointLight1 := light.NewPoint(&math32.Color{1, 1, 1}, 2.0) // Aumenta l'intensità della luce puntuale
...
func (r *G3NRenderer) RenderBody(b body.Body) {
	...
	if _, exists := r.bodyNodes[b.ID()]; !exists {
		// Crea un nuovo nodo per il corpo
		node := r.createNodeForBody(b)
		r.bodyNodes[b.ID()] = node
		r.scene.Add(node)
	}

	// Aggiorna la posizione del nodo
	r.updateNodeFromBody(b)
}
...
func (r *G3NRenderer) RenderBodies(bodies []body.Body) {
	...
	// Rimuovi i nodi dei corpi che non esistono più
	r.nodeMutex.Lock()
	defer r.nodeMutex.Unlock()

	for id, node := range r.bodyNodes {
		if !currentIDs[id] {
			r.scene.Remove(node)
...
// createNodeForBody crea un nodo grafico per un corpo fisico
func (r *G3NRenderer) createNodeForBody(b body.Body) *core.Node {
	node := core.NewNode()

	// Crea una sfera per rappresentare il corpo
	radius := float64(b.Radius().Value())
	geom := geometry.NewSphere(radius, 32, 16)
	...
	if b.Material() != nil {
		...
		case "Iron":
			mat.SetColor(&math32.Color{0.6, 0.6, 0.6})
		case "Rock":
			mat.SetColor(&math32.Color{0.5, 0.3, 0.2})
		case "Ice":
			mat.SetColor(&math32.Color{0.8, 0.9, 1.0})
		case "Copper":
			mat.SetColor(&math32.Color{0.8, 0.5, 0.2})
		default:
			...
			mat.SetColor(&math32.Color{r, g, b})
		}
	}
	...
	// Aggiungi il mesh al nodo
	node.Add(mesh)

	return node
}

// updateNodeFromBody aggiorna un nodo grafico in base allo stato del corpo fisico
func (r *G3NRenderer) updateNodeFromBody(b body.Body) {
	node := r.bodyNodes[b.ID()]

	// Aggiorna la posizione
	pos := b.Position()
	node.SetPosition(float32(pos.X()), float32(pos.Y()), float32(pos.Z()))
}
...
func (r *G3NRenderer) Run(updateFunc func(deltaTime time.Duration)) {
	...
	r.app.Run(func(renderer *renderer.Renderer, deltaTime time.Duration) {
		...
		// Renderizza la scena
		// Nota: questo è l'unico punto in cui viene chiamato Render
		// quando si usa il metodo Run
