...
func main() {
	log.Println("Initializing Raylib Physics example")
	...
	// Create Raylib adapter
	adapter := raylib.NewRaylibAdapter(1200, 800, "Solar System - Raylib")
...
func createSolarSystem(w world.World) {
	...
	sun := body.NewRigidBody(
		...
		units.NewQuantity(5.0, units.Meter),                        // Solar radius (scaled)
	...
	createAsteroidBelt(w, 50, solarMass, 60.0, 80.0) // Pass solar mass as central mass
}
...
func createPlanet(w world.World, name string, mass, radius, distance, centralMass float64, orbitPlane vector.Vector3, color [3]float64) body.Body {
	// Calculate orbital velocity using the agnostic function from the force package
